...
def calculate_unicore_crc32(data: bytes) -> int:
    crc = 0
    for byte in data:
        crc = _unicore_crc_table[(crc ^ byte) & 0xFF] ^ (crc >> 8)
    return crc

class GnssDualSerialIO:
    def __init__(self, device: str = '/dev/robot-gnss-dual', baudrate: int = 921600, fifo_size: int = 100):
        self.device = device
    ...
    def open(self):
        ...
        now = datetime.now()
        date_dir = f"/data/robot/gnss-dual/{now.strftime('%Y-%m-%d')}"
    ...
    def send_data(self, data: bytes) -> bool:
        try:
            ...
            print("[GnssDualSerialIO] Write FIFO full - dropping data!", file=sys.stderr)
            return False
        ...
        while not self._stop_event.is_set():
            ...
            while not self._write_fifo.empty():
                ...
                try:
                    ...
                    print(f"[GnssDualSerialIO] Writer error: {e}", file=sys.stderr)
                    ...
                    if line_str.startswith('$'):
                        ...
                        if '*' in line_str:
                            ...
                            if f"{calc_cs:02X}" == checksum.upper():
                                ...
                                print(f"[GnssDualSerialIO] Checksum error NMEA: {line_str}", file=sys.stderr)
                        ...
                        if '*' in line_str:
                            ...
                            calc_crc = calculate_unicore_crc32(crc_data)
                            if f"{calc_crc:08x}".upper() == checksum.upper():
                                self._fifo.put_nowait(line_str)
                            else:
                                self._err_checksum += 1
                                print(f"[GnssDualSerialIO] Checksum error Unicore: {line_str}", file=sys.stderr)
            except queue.Full:
                print("[GnssDualSerialIO] FIFO full - dropping sentence", file=sys.stderr)
            except Exception as e:
                if not self._stop_event.is_set():
                    print(f"[GnssDualSerialIO] Reader error: {e}", file=sys.stderr)
