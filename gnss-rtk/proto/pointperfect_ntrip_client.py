...
"""
    ...
    Typical command line examples for PointPerfect Flex NTRIP service:

    Using SPARTN corrections:
    python pointperfect_ntrip_client.py -P <port> -u <user> -p <password> --flex SPARTN

    Using RTCM corrections:
    python pointperfect_ntrip_client.py -P <port> -u <user> -p <password> --flex RTCM

    Using SPARTN continental corrections for EU (deprecated):
    ...
    <port> is the serial port of the GNSS receiver.
...
SOCKET_MAX_RECV_TIMEOUTS = 6   # number of timeouts before reconnecting
        ...
        while self.streaming:
            ...
            try:
                ...
                while self.streaming:
                    try:
                        ...
                        timeouts += 1
                        if timeouts >= SOCKET_MAX_RECV_TIMEOUTS:
                            logging.warning("No data received for %d seconds, reconnecting",
                                            SOCKET_TIMEOUT * SOCKET_MAX_RECV_TIMEOUTS)
                            break
                ...
                delay = SOCKET_RECONNECT_DELAY
                logging.warning("Connection lost, reconnecting in %d seconds", delay)
            except ssl.SSLError as e:
                delay = SERVER_RECONNECT_DELAY
                logging.warning("SSL error: %s, retrying in %d seconds", e, delay)
            except Exception as e:
                delay = -1
                logging.error("Unexpected error: %s", e)

            # hide the socket, so no GGA can be written
            self.sock = None
            ...
            if sock:
                try:
                    ...
                    pass
            if delay < 0:
                logging.error("Fatal error, stopping stream")
                sys.exit(1)
            else:
                time.sleep(delay)
...
class PointPerfectClient:
    ...
    Connects to the NTRIP caster and sends corrections to a GNSS receiver.
    """
...
def main():
    ...
    argp.add_argument("-V", "--version", action="version", version="BLD36-v1.12.0")
    argp.add_argument(
        "-P",
        "--port",
        required=True,
        help="Serial port[@baudrate] or pyserial URL of GNSS receiver",
    )

    s_group = argp.add_argument_group("Shortcut options")
    s_group.add_argument(
        "--flex", choices=("SPARTN", "RTCM"),
        default=argparse.SUPPRESS,
        help="Shortcut for PointPerfect Flex SPARTN or RTCM service. " +
             "Sets appropriate server, mountpoint, TLS and GGA interval options if not explicitly given."
    )
    ...
    tlsgroup = cgroup.add_mutually_exclusive_group()
    tlsgroup.add_argument(
        "-t", "--tls", action="store_true", help="Use secure (TLS) connection"
    )
    tlsgroup.add_argument(
        "--no-tls", action="store_true", help="Explicitly disable TLS"
    )
    cgroup.add_argument(
        "-m",
        "--mountpoint",
    ...
    argp.epilog = textwrap.dedent("""
        Example command line for PointPerfect Flex SPARTN service:
        $ python pointperfect_ntrip_client.py -P <port> -u <user> -p <password> --flex SPARTN

        Example command line for PointPerfect Flex RTCM service:
        $ python pointperfect_ntrip_client.py -P <port> -u <user> -p <password> --flex RTCM
    """)
    ...
    if hasattr(args, "flex"):
        if not args.tls and not args.no_tls:
            args.tls = True
            logging.info("Enabling TLS for PointPerfect Flex")
        if args.mountpoint is None:
            if args.flex == "SPARTN":
                args.mountpoint = "NEAR-SPARTN"
            else:
                args.mountpoint = "NEAR-RTCM"
            logging.info("Setting mountpoint to %s for PointPerfect Flex", args.mountpoint)
        if not args.ggainterval:
            args.ggainterval = 50
            logging.info("Setting GGA interval to %d for PointPerfect Flex", args.ggainterval)
    ...
    if not hasattr(args, "server"):
        ...
        logging.info("Using default NTRIP server %s:%d", ntrip_server, ntrip_port)
    else:
    ...
    if '://' in args.port:
        gnss = serial.serial_for_url(args.port, timeout=0.1)
    else:
    ...
    if args.mountpoint is None:
        args.mountpoint = ""  # Request sourcetable
