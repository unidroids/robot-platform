...
class LidarClient:
    ...
    async def send_command(self, cmd: str, timeout: float = 3.0) -> str:
        """Odešle příkaz TCP službě lidaru na zadaném portu a vrátí odpověď s timeoutem."""
        try:
            async def _interact():
            ...
            return response if response else "ERR: No response from lidar service"
        except asyncio.TimeoutError:
            print(f"[LidarClient][WARNING] Timeout waiting for lidar response (cmd: {cmd})")
            return "ERR: Timeout (3s)"
        except Exception as e:
            print(f"[LidarClient][ERROR] Error communicating with lidar service: {e}")
            return f"ERR: {e}"

    async def lidar_on(self, timeout: float = 3.0) -> str:
        return await self.send_command("START", timeout=timeout)

    async def lidar_off(self, timeout: float = 3.0) -> str:
