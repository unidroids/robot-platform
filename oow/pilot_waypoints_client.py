...
class PilotWaypointsClient:
    def __init__(self, host="127.0.0.1", port=9101):
        self.host = host
        self.port = port

    async def send_command(self, cmd: str, timeout: float = 3.0) -> str:
        """Odešle příkaz TCP službě pilot_waypoints na zadaném portu a vrátí odpověď s timeoutem."""
        try:
            async def _interact():
            ...
            return response if response else "ERR: No response from pilot_waypoints service"
        except asyncio.TimeoutError:
            print(f"[PilotWaypointsClient][WARNING] Timeout waiting for pilot_waypoints response (cmd: {cmd})")
            return "ERR: Timeout (3s)"
        except Exception as e:
            print(f"[PilotWaypointsClient][ERROR] Error communicating with pilot_waypoints service: {e}")
            return f"ERR: {e}"
