...
class DataLogger:
    def __init__(self, base_dir="/data/robot/pilot_robotour"):
        now = datetime.now()
        base = Path(base_dir)
        try:
            ...
            # Fallback na lokální složku projektu při absenci práv k /data
            local_base = Path(os.path.dirname(os.path.abspath(__file__))) / "data" / "robot" / "pilot_robotour"
            day_dir = local_base / now.strftime("%Y-%m-%d")
            day_dir.mkdir(parents=True, exist_ok=True)
            self.file_path = day_dir / f"navigate-{now.strftime('%H-%M-%S')}.csv"
            self.f = open(self.file_path, "w", encoding="utf-8")
